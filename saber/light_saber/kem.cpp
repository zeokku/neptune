...
int light_saber_kem_keypair(unsigned char *pk, unsigned char *sk)
{
  ...
  randombytes(sk + LIGHT_SABER_SECRETKEYBYTES - LIGHT_SABER_KEYBYTES, LIGHT_SABER_KEYBYTES); // Remaining part of sk contains a pseudo-random number.
                                                                                             // This is output when check in light_saber_kem_dec() fails.
  return (0);
}

int light_saber_kem_enc(unsigned char *c, unsigned char *k, const unsigned char *pk)
{
...
int light_saber_kem_dec(unsigned char *k, const unsigned char *c, const unsigned char *sk)
{
  int i, fail;
