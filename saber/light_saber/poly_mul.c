...
#define N_RES (LIGHT_SABER_N << 1)
#define N_SB (LIGHT_SABER_N >> 2)
...
{
    uint16_t c[2 * LIGHT_SABER_N] = {0};
    int i;

    toom_cook_4way(a, b, c);

    /* reduction */
    for (i = LIGHT_SABER_N; i < 2 * LIGHT_SABER_N; i++)
    {
        res[i - LIGHT_SABER_N] += (c[i - LIGHT_SABER_N] - c[i]);
    }
}
