...
#elif FIRE_SABER_ET == 4
	for (j = 0; j < FIRE_SABER_N / 2; j++)
	{
		offset_byte = j;
		offset_data = 2 * j;
		bytes[offset_byte] = (data[offset_data] & 0x0f) | ((data[offset_data + 1] & 0x0f) << 4);
	}
#elif FIRE_SABER_ET == 6
	for (j = 0; j < FIRE_SABER_N / 4; j++)
	{
		offset_byte = 3 * j;
		offset_data = 4 * j;
...
static void POLq2BS(uint8_t bytes[FIRE_SABER_POLYBYTES], const uint16_t data[FIRE_SABER_N])
{
	size_t j, offset_byte, offset_data;
	for (j = 0; j < FIRE_SABER_N / 8; j++)
	{
...
static void BS2POLq(const uint8_t bytes[FIRE_SABER_POLYBYTES], uint16_t data[FIRE_SABER_N])
{
	size_t j, offset_byte, offset_data;
	for (j = 0; j < FIRE_SABER_N / 8; j++)
	{
...
static void POLp2BS(uint8_t bytes[FIRE_SABER_POLYCOMPRESSEDBYTES], const uint16_t data[FIRE_SABER_N])
{
	size_t j, offset_byte, offset_data;
	for (j = 0; j < FIRE_SABER_N / 4; j++)
	{
...
static void BS2POLp(const uint8_t bytes[FIRE_SABER_POLYCOMPRESSEDBYTES], uint16_t data[FIRE_SABER_N])
{
	size_t j, offset_byte, offset_data;
	for (j = 0; j < FIRE_SABER_N / 4; j++)
	{
...
void POLVECq2BS(uint8_t bytes[FIRE_SABER_POLYVECBYTES], const uint16_t data[FIRE_SABER_L][FIRE_SABER_N])
{
	...
	{
		POLq2BS(bytes + i * FIRE_SABER_POLYBYTES, data[i]);
	}
}
...
void BS2POLVECq(const uint8_t bytes[FIRE_SABER_POLYVECBYTES], uint16_t data[FIRE_SABER_L][FIRE_SABER_N])
{
	...
	for (i = 0; i < FIRE_SABER_L; i++)
	{
		BS2POLq(bytes + i * FIRE_SABER_POLYBYTES, data[i]);
	}
}
...
void POLVECp2BS(uint8_t bytes[FIRE_SABER_POLYVECCOMPRESSEDBYTES], const uint16_t data[FIRE_SABER_L][FIRE_SABER_N])
{
	...
	for (i = 0; i < FIRE_SABER_L; i++)
	{
		POLp2BS(bytes + i * (FIRE_SABER_EP * FIRE_SABER_N / 8), data[i]);
	}
}
...
void BS2POLVECp(const uint8_t bytes[FIRE_SABER_POLYVECCOMPRESSEDBYTES], uint16_t data[FIRE_SABER_L][FIRE_SABER_N])
{
	...
	for (i = 0; i < FIRE_SABER_L; i++)
	{
		BS2POLp(bytes + i * (FIRE_SABER_EP * FIRE_SABER_N / 8), data[i]);
	}
}
...
{
	size_t i, j;
	for (j = 0; j < FIRE_SABER_KEYBYTES; j++)
	{
		for (i = 0; i < 8; i++)
		{
...
{
	size_t i, j;
	memset(bytes, 0, FIRE_SABER_KEYBYTES);
