...
#define N_RES (FIRE_SABER_N << 1)
#define N_SB (FIRE_SABER_N >> 2)
...
{
    uint16_t c[2 * FIRE_SABER_N] = {0};
    int i;

    toom_cook_4way(a, b, c);

    /* reduction */
    for (i = FIRE_SABER_N; i < 2 * FIRE_SABER_N; i++)
    {
        res[i - FIRE_SABER_N] += (c[i - FIRE_SABER_N] - c[i]);
    }
}
