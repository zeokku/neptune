                         ...
                         byte *fileBuf, int fileLen, byte *verifyFileBuf, int verifyFileLen)
{
    ...
    rsaKeyBuf = (byte *)malloc(rsaKeyLen);
    ...
    rsaPubKeyBuf = (byte *)malloc(rsaPubKeyLen);
...
int main(int argc, char **argv)
{
    ...
    if (argc >= 3)
    {
        sig_type = (wc_SignatureType)atoi(argv[2]);
    }
    if (argc >= 4)
    {
        hash_type = (wc_HashType)atoi(argv[3]);
    }
    if (argc >= 5)
    {
        verify_file = argv[4];
